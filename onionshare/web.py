...
def debug_mode():
    ...
    log_handler = logging.FileHandler(
        '{0}/onionshare_server.log'.format(temp_dir))
    ...
    def generate():
        ...
        while not done:
            ...
            if chunk == '':
                ...
                percent = round(
                    (1.0 * downloaded_bytes / zip_filesize) * 100, 2)
                sys.stdout.write(
                    "\r{0}, {1}%          ".format(
...
def page_not_found(e):
    ...
    return render_template_string(
        open('{0}/404.html'.format(helpers.get_onionshare_dir())).read())

# shutting down the server only works within the context of flask, so the
# easiest way to do it is over http
shutdown_slug = helpers.random_string(16)


@app.route("/<shutdown_slug_candidate>/shutdown")
def shutdown(shutdown_slug_candidate):
    if not helpers.constant_time_compare(shutdown_slug.encode('ascii'),
                                         shutdown_slug_candidate.encode(
                                             'ascii')):
        abort(404)
...
def stop(port):
    ...
    if helpers.get_platform() == 'Tails':
        # in Tails everything is proxies over Tor, so we need to get lower
        # level to connect not over the proxy
        import socket
        s = socket.socket()
        ...
        urllib2.urlopen(
            'http://127.0.0.1:{0}/{1}/shutdown'.format(
                port, shutdown_slug)).read()
